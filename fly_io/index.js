// Fly.io — MCP JSON-RPC stateless handler
// Runtime: Node.js
...
const TOOL = {
  ...
  description: "Returns the IP address of the MCP client initiating the request to this remote MCP server hosted on Fly.io.",
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === "initialize") {
    return sendJson(res, { jsonrpc: "2.0", id, result: {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "remote_mcp_server_fly_io", version: "1.0.0" },
    }}, startMs);
  }
  ...
  if (method === "tools/call" && params?.name === "get_mcp_client_ip") {
    const clientIp =
      ...
      "unknown";
    return sendJson(res, { jsonrpc: "2.0", id, result: {
      content: [{ type: "text", text: JSON.stringify({ mcp_client_ip: clientIp, hosting_provider: "fly_io" }) }],
    }}, startMs);
  }
...
server.listen(PORT, () => {
  console.log(`remote_mcp_server_fly_io listening on port ${PORT}`);
});
